...
python-security-check: base
	docker-compose -f docker-compose.backend.yml exec -T uplifty poetry run safety check -i 51457 -i 51499 -i 62044
...
# Run database migrations.
python-schema: base
	docker-compose -f docker-compose.backend.yml exec -T uplifty python manage.py export_schema uplifty.schema:schema > schema.graphql
